...
SELECT	category_id, product_id,
		COUNT(*) OVER() NOTHING, --bu tum satir sayimizi veriyor
		COUNT(*) OVER(PARTITION BY category_id) countofprod_by_cat, --category id de toplam kac tane varverdi
		COUNT(*) OVER(PARTITION BY category_id ORDER BY product_id ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) whole_rows, --buradada aynisini verir daraltma yok
		COUNT(*) OVER(PARTITION BY category_id ORDER BY product_id) countofprod_by_cat_2, ---orderbydan sonra rows tanimlamasi yapmadigimiz icin defaultumuz bunun bewtwwni unboundedprecidingden currentrowa kdardir
		COUNT(*) OVER(PARTITION BY category_id ORDER BY product_id ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) prev_with_current,--yazsakta olur yazmasakta yukarida bahsettigimiz gibi defaultumuz bu
		COUNT(*) OVER(PARTITION BY category_id ORDER BY product_id ROWS BETWEEN CURRENT ROW AND UNBOUNDED FOLLOWING) current_with_following,--ana rowumuzdan basla asagiya kdr al
		COUNT(*) OVER(PARTITION BY category_id ORDER BY product_id ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING) specified_columns_1,--kendisinden bir geri bak ve 1 ileri yani kendisiyle beraber 3
		COUNT(*) OVER(PARTITION BY category_id ORDER BY product_id ROWS BETWEEN 2 PRECEDING AND 3 FOLLOWING) specified_columns_2--2 geriye bak uc ileri ve kendisinide ekle 6 onunde yoksa 2 ileri yerine 1 sayar
FROM	product.product
...
---buradaa overin icine partition by yazmamiza gerek yok cunku tablomuzun icindeki en dusuk fiyati istiyor

SELECT distinct MIN(list_price) OVER() as cheapest
from product.product
...
--burada count icinde distinct kullanabiliyorken over buna izin vermez
...
--aggrigate fonksiyonlari tum framemim icinden bir ozet veriyordu bana
--navigation funksiyonlarimda o framein icinde geziyor hangi kural tanimlandiysa onu getiriryor
...
--buna model yeari en yeni olanida ekledik
select	distinct first_value(product_name) over (order by list_price, model_year DESC) cheapest_product
from	product.product
;

-- Liste fiyatı en düşük olan ürünün adını v e fiyatını getiriniz.
select	distinct
...
;
-----------------------LAG() VE LEAD()-----------------------------------------------
---BIR ONCEKI KAYITLARA ULASMAK ICIN LAG()
--BIR SONRAKI KAYITLARA ULASMAK ICIN LEAD()
--IKI SATIR ONCESINE GITMEK ICIN LAG(ORDER_DATE, 2 ) SEKLINDE IFADE EDEBILIRIZ



-- Çalışanların satış yaptıkları tarihleri listeleyin ve ayrıca bir önceki satış tarihlerini de yanına yazdırınız.

----calisanlarim hangi tarihte satis yaptiklari geldi bu asagidakiyle


SELECT b.order_id,a.staff_id,a.first_name,a.last_name ,b.order_date
FROM sale.staff a, sale.orders b
where a.staff_id=b.staff_id
order by a.staff_id,b.order_id


----simdi herbir staffin orderlarinin bir onceki tarihini yeni sutun olarak istiyoruz
